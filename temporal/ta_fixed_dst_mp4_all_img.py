...
def overlay_text(cvimg, text, subtext=None):
    ...
    if subtext:
        draw.text((50, 150), subtext, font=font, fill=(0, 255, 255))
    return cv2.cvtColor(np.array(pil_img), cv2.COLOR_RGB2BGR)

def extract_month_label(path):
    return os.path.basename(os.path.dirname(path))[:7]
...
# TO ALIGN ALL THE IMAGES FROM A CERTAIN I/P FOLDER
def collect_first_images_by_plot():
    plot_image_map = {plot: [] for plot in plot_numbers}
    for folder in input_folders:
        all_images = glob(os.path.join(folder, "*.JPG"))
        for img_path in all_images:
            try:
    ...
    # Sort each plot’s images by date extracted from folder name
    def extract_date(path):
        folder = os.path.basename(os.path.dirname(path))  # e.g., "2024-07-19_Renamed"
        return folder.split("_")[0]  # returns "2024-07-19"

    for plot_id in plot_image_map:
        plot_image_map[plot_id].sort(key=extract_date)
...
def process_plot_video(plot_no, image_paths):
    ...
    for idx, path in enumerate(rgb_paths):
        ...
        labeled = overlay_text(warped, label, subtext=month)
