...
output_folder = "/home/AD.UNLV.EDU/bhattb3/snow_melt/temporal_harris_combined_final/"
...
def auto_align_images(
    ...
    if downsample_scale < 1.0:
        ref_ds = cv2.resize(ref_img, None, fx=downsample_scale, fy=downsample_scale, interpolation=cv2.INTER_AREA)
        align_ds = cv2.resize(img_to_align, None, fx=downsample_scale, fy=downsample_scale, interpolation=cv2.INTER_AREA)
    else:
        ref_ds = ref_img
    ...
    if des1 is None or des2 is None or len(kp1) < 5 or len(kp2) < 5:
        print("  ORB: not enough features.")
        return None

    bf = cv2.BFMatcher(cv2.NORM_HAMMING, crossCheck=True)
    matches = bf.match(des1, des2)
    if len(matches) < min_inliers:
        print(f"  ORB: only {len(matches)} matches, too few.")
        return None
    ...
    if inliers is not None and np.sum(inliers) < min_inliers:
        print(f"  Homography rejected: {np.sum(inliers)} inliers.")
        return None
    ...
    shift_fullres = shift / downsample_scale

    if shift_fullres > max_corner_shift:
        print(f"  Corner shift {shift_fullres:.1f}px > {max_corner_shift}px")
        return None

    # Scale transformation back to full resolution if needed
    if downsample_scale < 1.0:
        M[0, 2] /= downsample_scale
        M[1, 2] /= downsample_scale

    aligned_img = cv2.warpAffine(img_to_align, M, (ref_img.shape[1], ref_img.shape[0]))
    return aligned_img
...
def process_images_auto(image_paths):
    ...
    if len(warped_images) < 2:
        print(" Not enough aligned images to average.")
        return None
...
def superimpose_final():
    for image_type in ["RGB", "THERMAL"]:
        ...
        print(f"? Saved final superimposed {image_type} overlay.")

# --- MAIN ---
if __name__ == "__main__":
    print("?? Starting enhanced temporal alignment with quality checks...\n")
